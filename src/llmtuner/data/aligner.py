...
def convert_sharegpt(examples: Dict[str, List[Any]], dataset_attr: "DatasetAttr") -> Dict[str, List[Any]]:
    ...
    accept_tags = (
        dataset_attr.user_tag, dataset_attr.observation_tag,
        dataset_attr.assistant_tag, dataset_attr.function_tag,
    )
    for i, messages in enumerate(examples[dataset_attr.messages]):
        ...
        for turn_idx, message in enumerate(messages):
            if message[dataset_attr.role_tag] not in accept_tags:
                raise ValueError("Invalid role tag in {}.".format(message))
