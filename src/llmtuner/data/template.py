    ...
    def _make_pairs(
        ...
        for i in range(0, len(encoded_messages), 2):
            ...
            max_source_len, max_target_len = infer_max_len(
                ...
                target_len=len(encoded_messages[i + 1]) if i+1 < len(encoded_messages) else 0,
                max_len=(cutoff_len - total_length),
                reserved_label_len=reserved_label_len,
            )
            source_ids = encoded_messages[i][:max_source_len]
            #TODO: fix bug
            target_ids = encoded_messages[i + 1][:max_target_len] if i+1 < len(encoded_messages) else []
...
_register_template(
    ...
    format_user=StringFormatter(slots=["USER: {{content}}", "ASSISTANT: "]),
    format_assistant=StringFormatter(slots=["{{content}}", {"eos_token"}]),
    format_function = FunctionFormatter(slots=["Action: {{name}}\nAction Input: {{arguments}}", {"eos_token"}]),
